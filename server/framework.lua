...
local query = {
    ['qbox'] = {
        ...
        queryStore2 = 'UPDATE `player_vehicles` SET `garage` = ?, `state` = 1, `mods` = ?, type = ? WHERE `plate` = ? ',
        queryImpound = 'UPDATE `player_vehicles` SET `garage` = ?, `state` = 0, `pound` = 1, `coords` = NULL, metadata = ? WHERE `plate` = ?'
    },
}
