...
lib.callback.register('mGarage:Interact', function(source, action, data, vehicle)
    ...
    local citizenid = Player.citizenid() ---ADICIONEI
    ...
    if action == 'get' then
        ...
        if PlyVehicles then
            for i = 1, #PlyVehicles do
                ...
                row.isOwner = row.owner == citizenid or row.license == identifier
    ...
    elseif action == 'spawn' then
        ...
        if vehicle then
            ...
            vehicle.vehicle = json.decode(vehicle.mods) ---mudei 
        ...
        if vehicle then
            ...
            if metadata.pound then
                if metadata.pound.endPound then
                    ...
                    MySQL.update('UPDATE player_vehicles SET metadata = ? WHERE plate = ?', {
    ...
    elseif action == 'changeName' then
        ...
        if Vehicle then
            ...
            local row = MySQL.single.await('SELECT `metadata` FROM `player_vehicles` WHERE `plate` = ? LIMIT 1', {
                data.vehicle.plate
            })
            if row then
