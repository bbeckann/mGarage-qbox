        ...
        if getVehicles.vehicles then
            for i = 1, #getVehicles.vehicles do
                ...
                if not row.private then
                    local props = json.decode(row.mods)  --APENAS MUDEI 
                    ...
                    if data.garagetype == 'impound' then
                        if row.pound and row.state == 0 then -- mudei row.state
...
local blipcar = function(coords, plate)
    if coords == nil then
        -- Exibe uma mensagem informando que as coordenadas do GPS não foram encontradas
        Notification({
            title = 'GPS',
            description = 'Coordenadas do GPS não encontradas.',
            type = 'warning',
        })
        return false
    end
    ...
    AddTextComponentString('GPS - ' .. plate)
